...
import { syncMcpPatchEntries, type PatchEntry } from "./patch.js";
 ...
 */
export function profilePatchPath(includeEntry: Entry): string {
...
function splitLeadingComments(content: string): { header: string; body: string } {
...
export function parsePatchEntries(content: string): PatchEntry[] {
 ...
 */
export async function persistMcpPatch(includeEntry: Entry, rows: EntryOptions[]): Promise<void> {
