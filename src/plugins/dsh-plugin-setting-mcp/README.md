...
- **Save → hot reload** — edits are staged locally; **Save** reconciles the
  loader tree (`create` / `update` / `remove`) and writes it back to the backing
  config file. Each operation restarts only the affected `dsh-mcp-client`
  instance, which is the same hot-swap the harness HMR performs.
...
- **`reconnect` is not surfaced by the editor.** A disabled/enabled toggle keeps
  the stored `reconnect` block; an edit rebuilds the transport config and resets
  `reconnect` to `dsh-mcp-client`'s defaults.
